...
func main() {
	...
	// GET: FollowedTopics
	r.HandleFunc("/users/{userId}/topics", dbServer.GetFollowedTopicsHandler)
	// POST: Post
	...
	// PUT: Follow Topic
	r.Path("/users/{userId}/topics/add").Queries("topic", "{topic}").HandlerFunc(dbServer.FollowTopicHandler)
